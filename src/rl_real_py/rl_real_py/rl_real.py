...
class RL_real(Node):
    def __init__(self,name):
        ...
        with open(config_path,'r') as f:
            ...
            self.policy_path = os.path.join(package_path,
            '..', '..', '..', '..',
            'src',
            'rl_real_py',config['policy_path'])
        ...
        if self.model_type == "jit":
            ...
            # self.policy_path = "/home/woan/rl_real/src/rl_real_py/policy/policy_lab_0313_2(p10d01).onnx"
    ...
    def joy_callback(self, msg):
        ...
        if abs(msg.axes[2]) > self.deadzone:
            self.yaw = msg.axes[2] * self.speed_scale * 2.0 # 反转转向方向
        else:
            self.yaw = 0.0
    ...
    def timer_callback(self):
        ...
        if self.reset_symbol:
            ...
            self.yaw = 0.
            self.reset_symbol = False
        # if self.pause_symbol and self.obs_get:
        if self.pause_symbol:
            # if self.stand_symbol:
            #     start_pos = self.lab2real(self.get_obs[7:19])
            #     self.stand(start_pos)
            #     self.stand_symbol = False
            self.commands = np.array(self.lab2real(self.default_angles),dtype=np.float32)
            print('pause                          ',end='\r')
        else:
