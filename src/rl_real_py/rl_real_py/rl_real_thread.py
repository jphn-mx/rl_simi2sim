...
class RL_real(Node):
    ...
    def commands_thread(self):
        while rclpy.ok():
            ...
            with self.lock:
                ...
                self.commands[1] = np.clip(commands[1],-0.1,2.)
                self.commands[4] = np.clip(commands[4],-0.1,2.)
                self.commands[7] = np.clip(commands[7],-0.1,2.)
                self.commands[10] = np.clip(commands[10],-0.1,2.)

                self.commands[2] = np.clip(commands[2],-2.,-0.6)
                self.commands[5] = np.clip(commands[5],-2.,-0.6)
                self.commands[8] = np.clip(commands[8],-2.,-0.6)
                self.commands[11] = np.clip(commands[11],-2.,-0.6) ## clip
                msg_data = np.array(self.commands).tolist()
    ...
    def model_thread(self):
        while rclpy.ok():
            ...
            end_time1 = time.time()
            print("update time:",end_time1-start_time1)
            start_time2 = time.time()
            if self.model_type == "jit":
                new_actions = torch.clip(self.policy(obs_tensor.float()),-100.0,100.0).detach().numpy().squeeze()
            elif self.model_type == "onnx":
                # ONNX 推理
                ...
                new_actions = np.clip(outputs[0], -100.0, 100.0).squeeze()
            end_time2 = time.time()
            print("policy time:",end_time2-start_time2) 
    ...
    def timer_callback(self):
        ...
        if self.counter % self.control_decimation == 0:
            ...
            # self.commands[np.arange(0, 12, 3)] = np.clip(commands[np.arange(0, 12, 3)], -0.4, 0.4)
            # self.commands[np.arange(1, 12, 3)] = np.clip(commands[np.arange(1, 12, 3)], -0.8, 2.0)
            # self.commands[np.arange(2, 12, 3)] = np.clip(commands[np.arange(2, 12, 3)], -2.0, -0.8) 
        ...
        # test = [0.0, 0.8000, -1.5, 0.0, 0.800, -1.5, 0.0, 0.8000000, -1.5, 0.0, 0.800000, -1.5]
        # msg.data = [float(x) for x in test]
        # print("commands pos",msg.data)
    ...
    def obs_callback(self,msg):
        with self.lock:
            ...
            self.get_obs[19:31] = self.real2lab([msg.velocity[0],msg.velocity[1],msg.velocity[2],
                                    ...
                                    msg.velocity[6],msg.velocity[7],msg.velocity[8],
                                    msg.velocity[9],msg.velocity[10],msg.velocity[11]])
        # print(msg.velocity[0],msg.velocity[3],msg.velocity[6],msg.velocity[9])
        # print(f"get obs:\n{msg.data}")
