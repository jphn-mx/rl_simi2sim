...
class RL_real(Node):
    def __init__(self,name):
        ...
        # 提取包路径（用于加载模型）
        package_path = os.path.dirname(os.path.dirname(config_path))
    ...
    def model_thread(self):
        while rclpy.ok():
            ...
            end_time1 = time.time()
            # print("update time:",end_time1-start_time)
            ...
            if self.model_type == "jit":
                ...
                input_data = np.clip(total_obs[np.newaxis, ...], -100.0, 100.0).astype(np.float32)
                outputs = self.policy.run(None, {input_name: input_data})
                new_actions = np.clip(outputs[0], -100.0, 100.0).squeeze()
            
            end_time2 = time.time()
            # print(f"policy time: {end_time2-start_time2} (using {self.model_type})") 
    ...
    def joy_callback(self, msg):
        ...
        if abs(msg.axes[3]) > self.deadzone:
            self.yaw = -msg.axes[3] * self.speed_scale  # 反转转向方向
        else:
            self.yaw = 0.0
